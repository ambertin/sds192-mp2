...
### Plot total donations by occupation
```{r}

ggplot(donations_by_occupations, aes(x=reorder(occupation,total_donated_millions), y = total_donated_millions, fill = income_level)) +
  geom_bar(stat = "identity") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  xlab(NULL) +
  ylab("Total Donated (Millions of Dollars)") +
  ggtitle("Total Individual Contributions by Contributer Occupation")

```

### Plot avg donations by occupation

```{r}

ggplot(donations_by_occupations, aes(x=reorder(occupation,total_donated_millions), y = avg_donated, fill = income_level)) +
  geom_bar(stat = "identity") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  xlab(NULL) +
  ylab("Average Donation (Dollars)") +
  ggtitle("Average Individual Contributions by Contributer Occupation")
